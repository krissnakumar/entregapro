    ...
    <div className="space-y-8 pb-16 font-sans animate-in fade-in duration-300">
      ...
      <div className="bg-white border border-slate-200 p-6 rounded-[2rem] shadow-xs flex flex-wrap items-center justify-center gap-4 transition-all hover:border-slate-300">
        ...
        <button 
          ...
          className="bg-indigo-600 hover:bg-indigo-700 text-white px-6 py-3 rounded-xl font-bold text-xs uppercase tracking-wider shadow-md shadow-indigo-600/20 active:scale-95 transition-all flex items-center gap-2 cursor-pointer outline-none shrink-0"
        >
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-5 gap-3">
        {stats.map((stat) => (
          <div 
            ...
            className="bg-white border border-slate-200 p-3.5 rounded-2xl flex items-center gap-3 hover-lift transition-all cursor-pointer group overflow-hidden"
          ...
          >
            ...
            <div className="min-w-0 flex-1">
              <p className="text-[9px] font-bold uppercase tracking-widest text-slate-400 truncate leading-none mb-1 group-hover:text-indigo-600 transition-colors">
                {stat.title}
              </p>
              <p className={cn("text-base font-bold tracking-tight truncate", stat.color)}>
                {stat.value}
              </p>
            </div>
          </div>
        ...
        <div className="lg:col-span-2 bg-white border border-slate-200 rounded-[2.5rem] shadow-xs overflow-hidden flex flex-col h-[520px]">
          <div className="p-6 border-b border-slate-100 flex items-center justify-between bg-slate-50/50">
            <div className="flex items-center gap-2.5">
              ...
              <h3 className="font-bold text-xs uppercase tracking-widest text-slate-900 truncate">
                Operações de Pátio e Rodovia ao Vivo
              </h3>
            </div>
            <div className="flex items-center gap-3 text-[9px] font-bold uppercase tracking-widest text-slate-400 shrink-0">
              ...
              {topDriversList.map((driver: any, i: number) => (
                ...
                >
                  <div className="flex items-center gap-3 min-w-0">
                    <div className="w-8 h-8 rounded-lg bg-indigo-50 text-indigo-700 flex items-center justify-center font-bold text-xs group-hover:bg-indigo-600 group-hover:text-white transition-colors shrink-0">
                      {i + 1}
                    </div>
                  ...
                  <div className="text-emerald-600 flex items-center gap-1 font-bold text-xs font-mono bg-emerald-50 px-2 py-1 rounded-md shrink-0 border border-emerald-100/60">
                    <TrendingUp size={12} />
                    {driver.rate || '98.5%'}
                  </div>
                </div>
          ...
          <div className="bg-white border border-slate-200 rounded-[2.5rem] p-6 shadow-xs relative overflow-hidden transition-all hover:border-slate-300">
             <div className="relative z-10">
               <div className="flex items-center justify-between mb-1">
                 <h3 className="text-xs font-bold uppercase tracking-widest text-slate-900">Integridade da Frota</h3>
               ...
               <div className="space-y-2.5">
                 {maintenanceData?.slice(0, 2).map((v: any) => (
                   ...
                   >
                     ...
                     <span className="text-[8px] font-bold uppercase bg-amber-50 text-amber-700 px-2 py-0.5 rounded border border-amber-200">
                       Revisão Próxima
                     </span>
                   </div>
                 ))}
      ...
      <div className="bg-white border border-rose-200 rounded-[2.5rem] shadow-xs p-8">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-6">
          <div className="flex items-center gap-2.5">
            ...
            <div>
              <h3 className="font-bold text-xs uppercase tracking-widest text-rose-700">
          ...
          <span className="bg-rose-50 text-rose-700 border border-rose-200 text-[9px] font-bold px-2.5 py-1 rounded-lg uppercase tracking-wider self-start sm:self-auto shrink-0">
            SLA em Vigilância
          </span>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
           {deliveries.filter(d => d.status === 'DELAYED' || (d.status !== 'DELIVERED' && new Date(d.scheduledTime) < new Date())).slice(0, 2).map((delivery: any) => (
             ...
             >
               <div className="flex items-center gap-3 min-w-0">
                 <div className="w-10 h-10 bg-white text-rose-600 border border-rose-100 rounded-xl flex items-center justify-center font-bold text-sm shrink-0">
                   ⚠️
                 </div>
                 <div className="min-w-0">
                   <p className="font-bold text-xs text-slate-900 truncate">{delivery.customer?.name || 'Cliente Identificado'}</p>
      ...
      <div className="bg-white border border-slate-200 rounded-[2.5rem] p-8 space-y-8 shadow-xs relative overflow-hidden transition-all hover:border-slate-300">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-slate-100 pb-6">
          <div>
            <div className="flex items-center gap-2 mb-1">
              ...
              <span className="text-[9px] font-bold uppercase tracking-[0.2em] text-indigo-600">Sistemas Consolidados PostGIS</span>
            </div>
            <h3 className="font-bold text-xl tracking-tight text-slate-900">Central Gráfica de Comando Operacional</h3>
          ...
          <span className="px-3 py-1.5 bg-slate-50 text-indigo-700 border border-slate-200 rounded-xl text-[9px] font-bold uppercase tracking-widest shrink-0 text-center">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-6">
          ...
          <div className="bg-slate-50 p-5 rounded-[1.5rem] border border-slate-100 flex flex-col justify-between h-44 relative overflow-hidden group hover-lift transition-all">
            <div className="flex items-center justify-between z-10">
              <span className="text-[9px] font-bold text-slate-400 uppercase tracking-widest block truncate">Ocupação Espacial</span>
            ...
            <div className="z-10 mt-auto pt-2 border-t border-slate-200/60 flex items-center justify-between">
              ...
              <span className="text-[9px] font-bold font-mono text-indigo-700 shrink-0">92.4% Densidade</span>
            </div>
          </div>
          ...
          <div className="bg-slate-50 p-5 rounded-[1.5rem] border border-slate-100 flex flex-col justify-between h-44 relative overflow-hidden group hover-lift transition-all">
            <div className="flex items-center justify-between z-10">
              <span className="text-[9px] font-bold text-slate-400 uppercase tracking-widest block truncate">Cadência Veicular</span>
            ...
            <div className="flex items-center justify-around my-auto z-10 py-1">
              <div className="relative w-14 h-14 flex items-center justify-center bg-white rounded-full border border-slate-200 shadow-2xs">
                <span className="font-mono font-bold text-xs text-slate-900">88%</span>
              </div>
            ...
            <div className="z-10 mt-auto pt-2 border-t border-slate-200/60 flex items-center justify-between">
              ...
              <span className="text-[9px] font-bold text-amber-600 shrink-0">Otimizada</span>
            </div>
          </div>
          ...
          <div className="bg-slate-50 p-5 rounded-[1.5rem] border border-slate-100 flex flex-col justify-between h-44 relative overflow-hidden group hover-lift transition-all">
            <div className="flex items-center justify-between z-10">
              <span className="text-[9px] font-bold text-slate-400 uppercase tracking-widest block truncate">Volume Mês</span>
            ...
            <div className="z-10 mt-auto pt-2 border-t border-slate-200/60 flex items-center justify-between">
              ...
              <span className="text-[10px] font-bold font-mono text-purple-700 shrink-0">+18.4%</span>
            </div>
          </div>
          ...
          <div className="bg-slate-50 p-5 rounded-[1.5rem] border border-slate-100 flex flex-col justify-between h-44 relative overflow-hidden group hover-lift transition-all">
            <div className="flex items-center justify-between z-10">
              <span className="text-[9px] font-bold text-slate-400 uppercase tracking-widest block truncate">Combustão Diesel</span>
            ...
            <div className="my-auto z-10 space-y-2">
              <div className="flex items-baseline justify-between">
                <span className="text-xl font-bold font-mono text-slate-900 tracking-tight">4.5 <span className="text-[10px] text-cyan-600">L/km</span></span>
            ...
            <div className="z-10 mt-auto pt-2 border-t border-slate-200/60 flex items-center justify-between">
              ...
              <span className="text-[9px] font-bold text-cyan-600 font-mono shrink-0">Ativa</span>
            </div>
          </div>
        ...
        <div className="bg-slate-50 border border-slate-200 p-6 rounded-[1.5rem] flex flex-col sm:flex-row sm:items-center justify-between gap-4 hover-lift transition-all">
          <div className="space-y-1">
            <div className="flex items-center gap-1.5">
              <ShieldCheck size={14} className="text-emerald-600 shrink-0" />
              <span className="text-[9px] font-bold text-emerald-700 uppercase tracking-widest block truncate">Índice de Confiabilidade Contínua</span>
            </div>
          ...
          <div className="px-4 py-2.5 bg-white text-emerald-700 border border-emerald-200 rounded-xl font-bold text-xs uppercase tracking-widest shadow-2xs shrink-0 text-center self-start sm:self-auto">
            SLA Preservado
          </div>
        </div>
