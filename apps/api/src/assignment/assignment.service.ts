...
export class AssignmentService {
  ...
  async runAssignment(organizationId: string) {
    ...
    if (invoices.length === 0) {
      return { message: "Nenhuma entrega pendente para alocar.", recommendations: [] };
    }

    // Step 2: Get available drivers with vehicles
    const drivers = await this.getAvailableDrivers(organizationId);
    if (drivers.length === 0) {
      return { message: "Nenhum motorista disponivel no momento.", recommendations: [] };
    }
    ...
    const recommendations = await this.scoreAssignments(groups, drivers, organizationId);
    ...
    for (const rec of recommendations.slice(0, drivers.length * 2)) {
      ...
      await this.prisma.assignmentRecommendation.create({
        data: {
          ...
          vehicleId: rec.vehicleId,
          totalDistance: rec.totalDistance,
          totalDuration: Math.round((rec.totalDistance / this.AVG_SPEED_KMH) * 60 + rec.invoiceIds.length * this.STOP_TIME_MIN),
  ...
  private async validateOrders(organizationId: string) {
    ...
    const valid = invoices.filter((i) => {
      ...
      if (issues.length > 0) {
        this.logger.warn(`Invoice ${i.id} validation issues: ${issues.join(", ")}`);
        return false;
      }
      return true;
    });
  ...
  private async getAvailableDrivers(organizationId: string) {
    const drivers = await this.prisma.driver.findMany({
      ...
      include: {
        ...
        vehicle: {
          select: {
            id: true, vehicleNumber: true, capacity: true, activeStatus: true,
            vehicleCapacities: { select: { maxWeight: true, maxVolume: true, maxStops: true } },
          },
        },
        driverPerformance: { select: { score: true, onTimeRate: true, totalDeliveries: true } },
    ...
    return drivers.filter(
      (d: any) => d.vehicle?.activeStatus !== false && !busyDriverIds.includes(d.id),
    );
  }
  ...
  private groupByProximity(
    ...
    for (const inv of sorted) {
      ...
      for (const other of sorted) {
        ...
        const dist = this.haversine(inv.addressLat!, inv.addressLng!, other.addressLat!, other.addressLng!);
        if (dist <= maxRadiusKm) {
          group.push(other);
  ...
  private async scoreAssignments(
    ...
    for (const group of groups) {
      ...
      for (const driver of drivers) {
        const vc = (driver as any).vehicle?.vehicleCapacities?.[0];
        ...
        // Calculate distance
        let totalDistance = 0;
        let prevLat = (driver as any).liveLatitude ?? -23.5505;
        let prevLng = (driver as any).liveLongitude ?? -46.6333;

        const ordered = this.nearestNeighbor(group, prevLat, prevLng);

        for (const stop of ordered) {
          const dist = this.haversine(prevLat, prevLng, stop.addressLat!, stop.addressLng!);
          totalDistance += dist;
        ...
        const totalDurationMin =
          (totalDistance / this.AVG_SPEED_KMH) * 60 + group.length * this.STOP_TIME_MIN;
        ...
        const score = distanceScore + capacityScore + durationScore + perfScore + densityScore;
        ...
        const capacityUtil = maxWeight > 0 ? (totalWeight / maxWeight) * 100 : 0;
  ...
  private nearestNeighbor(points: any[], startLat: number, startLng: number) {
    ...
    while (unvisited.length > 0) {
      ...
      for (let i = 0; i < unvisited.length; i++) {
        const dist = this.haversine(lat, lng, unvisited[i].addressLat!, unvisited[i].addressLng!);
        if (dist < minDist) {
          minDist = dist;
          nearestIdx = i;
        }
